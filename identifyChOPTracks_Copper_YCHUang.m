...
function ChOPfinalTracks = identifyChOPTracks_Copper_YCHUang(linkedTracks,stimulusfile,stimtoIncl)
...
for (i=1:length(TracksInStim(:,1)))
    ...
    linkedTracks(TracksInStim(i,2)).stimulus_vector(((stimulus(currentStim,1)*3)-FirstFrame+1):((stimulus(currentStim,2)*3)-FirstFrame)) = 1;            
end

%Make output
ChOPTrackIndices = unique(TracksInStim(:,2));

ChOPfinalTracks = linkedTracks(ChOPTrackIndices);
