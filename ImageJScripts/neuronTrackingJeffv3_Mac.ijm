...
TrackSettingfile="/Volumes/Clone of Jeff #5/ImageJ/macros/TrckSett_awa_2p5x_4pxSqMoving_b.txt"; 
...
pathnoext = dir+"neuronTracking/"+substring(title,0,lengthOf(title)); 
...
if(anyKeyDown){ 
	...
	if (isKeyDown("space")) { 
		...
		do { 
			...
			if (isKeyDown("shift")) { 
				showStatus("Begin manual tracking");
				singleFrame=true;
				do{
					wait(50);
...
if (useTracking == 1) { 
	...
	if (!File.exists(logname)){ 
		...
		Dialog.addChoice("Choose:", newArray("New Animal", "Overwrite")); 
		Dialog.show(); 
