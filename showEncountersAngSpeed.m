function [refeeds, avgs] = showEncountersAngSpeed(tracks, varargin)
...
for s = n:length(strains)
    y = [avgs.(strains{s}).AngSpeed];
    start = find(~isnan(y), 1);
    fin = find(~isnan(y), 1, 'last');
    xThis = x(start:fin);
    y = y(start:fin);
    err = stdErr.(strains{s}).AngSpeed(start:fin);
    try
...
%         end
    ...
    errorshade(xThis,[y + err],[y - err],colors(c), nums/(max(nums)*3));
    patch([xThis NaN],[y NaN], colors(c),'EdgeColor',colors(c), 'DisplayName', sprintf('%s (n = %i)', strains{s}, length(refeeds.(strains{s}))),...
        'LineWidth', 1.5, 'FaceVertexAlphaData', [nums; NaN], 'EdgeAlpha', 'flat', 'AlphaDataMapping', 'direct')
...
function showSpeedBars(avgs, stdErr, lags, frameRate, wormCounts, strains)
    ...
    for t = 1:size(barTimes, 1)
        ...
        means(t,:) = arrayfun(@(s) nanmean(avgs.(strains{s}).AngSpeed(barFrames(t,1):barFrames(t,2))), 1:length(strains)).*1000;
        stdErry(t,:) = arrayfun(@(s) nanmean(stdErr.(strains{s}).AngSpeed(barFrames(t,1):barFrames(t,2))), 1:length(strains)).*1000;
