function fixVidMultiShift(vidFile, refFrameNum, shiftFrameNums)
...
%             [sub_shifted,rect_shifted] = imcrop(shiftFrame); %select sampling region        
            ...
            while max(c(:)) < 0.8
                ...
                c = normxcorr2(rgb2gray(sub_ref), rgb2gray(sub_shifted));

                % offset found by correlation
    %             [~, imax] = max(abs(c(:)));
    %             [ypeak, xpeak] = ind2sub(size(c),imax(1));
                [ypeak, xpeak] = find(c==max(c(:)));
                corr_offset = [(xpeak-size(sub_ref,2))
                               (ypeak-size(sub_ref,1))];

    %             relative offset of position of subimages
                rect_offset = [rect_shifted(1)-(rect_ref(1))
                               (rect_shifted(2)-rect_ref(2))];
        ...
        if s == size(shiftFrameNums,1)
            span = length([shiftFrameNums(s,2):v.NumberOfFrames]) - 1;
        else
            span = length([shiftFrameNums(s,2):shiftFrameNums(s+1,1)]) - 1;
        end
        refxbegin(f+1:f+1+span) = refxbegin(f);
        refxend(f+1:f+1+span)   = refxend(f);
        refybegin(f+1:f+1+span) = refybegin(f);
        refyend(f+1:f+1+span)   = refyend(f);

        shiftxbegin(f+1:f+1+span) = shiftxbegin(f);
        shiftxend(f+1:f+1+span)   = shiftxend(f);
        shiftybegin(f+1:f+1+span) = shiftybegin(f);
        shiftyend(f+1:f+1+span)   = shiftyend(f);
    end    
