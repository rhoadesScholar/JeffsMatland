function [strainMats, avgs, stdErr, wormsUsed] = showAvgCalTracks(calTracks, medWindow, varargin)
    
    
    global fedLimit
    fedLimit = 3;%set in minutes
...
%             avgs.(strains{s}).fluor = nanmedian(cleanTracks.fluors);
        ...
        if ~indie
            ...
            zt = find(t == 0);
            if contains(strainTracks(1).name, 'opto', 'IgnoreCase', true)
                xlim([0 20]);
                xlimit = [zt, zt+(10*60*20)];
            else
                xlimit = [zt-300, zt+2400];
            end
