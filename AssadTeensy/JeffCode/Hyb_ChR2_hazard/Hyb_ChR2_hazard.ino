...
static long _lastDelta   = 0;    // Keep track of the last analog read delta
static long _vel   = 0;    // Keep track of the difference between this analog read delta and the last
static long _lastVel   = 0;    // Keep track of the last analog read delta
...
void mySetup()
{
 ...
 if (_analog_lick_mode) {
    ...
    while (_thresh_setting){
        ...
        _cumulativeDelta = _cumulativeDelta + (_raw_read - _readBaseline);
        ...
        if ((_alick_timer + alick_delay <= signedMicros()) || (_alick_timer > signedMicros())) {
          ...
          if (_baselineTimer + filter_delay <= signedMillis()){
            ...
            _lastVel = (bias/100)*_lastVel + (1-(bias/100))*_vel;
          }
          
          _vel = (bias/100)*_vel + (1-(bias/100))*(_cumulativeDelta - _lastDelta);
          _accel = _vel - _lastVel;
          _lastDelta = (bias/100)*_lastDelta + (1-(bias/100))*_cumulativeDelta;
        }

        analogWrite(PIN_LICK_LED, int(255*(1-exp(-abs(_cumulativeDelta)))));
        digitalWrite(PIN_LICK_OUT, digitalRead(PIN_LICK));
        if (abs(_accel) > _params[A_LICK_THRESH]) {
...
bool getLickState() {
  if (_analog_lick_mode){
    ...
    analogWrite(PIN_LICK_LED, int(255*(1-exp(-abs(_cumulativeDelta)))));
    digitalWrite(PIN_LICK_OUT, digitalRead(PIN_LICK));
    return (abs(_accel) > _params[A_LICK_THRESH]);
	}
	else {
    digitalWrite(PIN_LICK_OUT, digitalRead(PIN_LICK));  
