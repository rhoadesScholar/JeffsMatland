	...
	methods
		function obj = ArduinoConnection(arduinoPortName)
            if strcmp(arduinoPortName, '/offline')
                ...
                %Prepare threshold setting plot
                obj.fig = figure;
                obj.fig.UserData = obj;
                obj.fig.KeyPressFcn = @threshPress;
                obj.fig.Interruptible = 'off';
                hold on
                title('delta of Lick Tube');
                xlabel('t');
                ylabel('d');
                obj.d = plot(0, 'LineWidth', 2);
                obj.d.BusyAction = 'cancel';
                obj.th = plot(0, 'LineWidth', 5, 'Color', 'g');                
                obj.th.BusyAction = 'cancel';
                set(gca, 'BusyAction', 'cancel');

%                 obj.thn = plot(0, 'LineWidth', 5, 'Color', 'g');                
%                 obj.thn.BusyAction = 'cancel';
                obj.threshSetting = true;
...
% 				end
                ...
                %Set analog lick threshold ------> ',' = -10; '.' = +10; 'k' = -1; 'l' = +1
%                 while (obj.threshSetting)
                waitfor(obj.fig);
%                     pause(0.01)
%                 end             
...
%                     fprintf('Made it here\n')
                    if isempty(obj.thn) || obj.thn > 50
                        obj.thn = 0;
                        drawnow limitrate
                    end
                    obj.thn = obj.thn + 1;
                    messages = strsplit(strtrim(messageString), ' ');
                    messages = messages(2:end);
                    if ~isempty(messages) && strcmp(messages{1}, '-') && length(messages) == 2
                            paramId = strcmp(obj.ParamNames, 'A_LICK_THRESH');
                            obj.ParamValues(paramId) = str2double(messages{2});
                            obj.threshSetting = false;
                            close(obj.fig)
                    elseif ~isempty(messages)
                        if length(obj.d.YData)>=1000
                            obj.d.YData = [obj.d.YData(2:end) str2double(messages{1})];
                            obj.th.YData = [obj.th.YData(2:end) str2double(messages{2})];                            
%                             obj.thn.YData = [obj.thn.YData(2:end) -str2double(messages{2})];
                        else
                            obj.d.YData(end + 1) = str2double(messages{1});
                            obj.th.YData(end + 1) = str2double(messages{2});
%                             obj.thn.YData(end + 1) = -str2double(messages{2});
                        end
                        if (abs(obj.d.YData(end)) > abs(obj.th.YData(end)))
                            obj.d.Color = 'r';
                            drawnow limitrate
                        else
                            obj.d.Color = 'b';                        
...
%---------------------------threshSetting methods        
function threshPress(src, event)
    %src.UserData.SendMessage(event.Character);
    fprintf(src.UserData.SerialConnection, event.Character, 'async');
    drawnow limitrate
%     disp(event.Key)
%     pause(0.01)
end
