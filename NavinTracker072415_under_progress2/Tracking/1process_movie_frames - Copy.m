...
%                 
%                 [cc, NumWorms, numClumps, numWormsClump] = worm_bwconncomp(Movsubtract, DefaultLevel );
%                 total_numWorms = NumWorms + numWormsClump;
%                 
%                 disp(num2str([Frame total_numWorms num_target_worms half_num_target_worms num_missing_worm_threshold DefaultLevel]))
%             end
%         end
%     end
    ...
    del_missing = 2;
    if(mod(p, frames_ten_sec)==0 || p==1)
        del_missing = 0;
    end
    
    % adjust overall threshold level for detecting missing animals
    if( ( abs( num_target_worms - total_numWorms ) >= num_missing_worm_threshold )  || (total_numWorms <= half_num_target_worms) || mod(p, frames_ten_sec)==0 || p==1)
        ...
        if( (abs(thresh_score(rr)) >= (num_missing_worm_threshold + del_missing)) || (total_worms_local(rr) <= half_num_target_worms) || mod(p, frames_ten_sec)==0 || p==1)
        ...
        idx = idx(1);
        %         if(best_score > num_missing_worm_threshold)
        %             [best_score, idx] = min((thresh_score));
        %             idx = idx(1);
        %         end
