function outstats = custom_regionprops(varargin) % regionprops w/ line 142 commented out; saves 40% time!
...
%   objects in BW, CC.NumObjects, or max(L(:)). The fields of the structure
%   array denote different properties for each region, as specified by
%   PROPERTIES. 
%
...
%   dimension. 
%
%   See also BWCONNCOMP,BWLABEL,BWLABELN,ISMEMBER,LABELMATRIX,WATERSHED.

%   Copyright 1993-2008 The MathWorks, Inc.
%   $Revision.4.2.3 $  $Date: 2008/10/26 14:24:23 $

% iptchecknargin(1, inf, nargin, mfilename); % commented out by Navin 1/6/13
...
if islogical(varargin{1}) || isstruct(varargin{1})
    ...
    L = varargin{1};
% this line seems to soak up ~40% of the runtime
...
if numObjs ~= 0
    if ~isempty(CC)
        ...
        idxList = regionpropsmex(L, numObjs);
    end
...
for k = 1 : numProps
    if ischar(propList{k})
        ...
        % commented out by Navin 1/16/13
...
%         end
%         reqStats{k} = prop;
        reqStats{k} = propList{k}; % added by Navin 1/16/13
    else
...
wid = sprintf('Images:%s:measurementNotForND',mfilename);
