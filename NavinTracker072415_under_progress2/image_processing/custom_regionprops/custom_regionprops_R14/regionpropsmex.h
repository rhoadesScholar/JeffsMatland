...
mxArray *compute(_T *lMatrixPtr)
{
    ...
    int               label;              //pixel label
    bool              *isInitialized;     //flag if sequence is initialized
    int               hint = 50;          //initial sequence length
    int               oneSeqLength = 0;   //length of one sequence
    ...
    int numElements = mxGetNumberOfElements(fLabel);
    double val;
    for (int p = 0; p < numElements; p++)
    {
        label = (int) lMatrixPtr[p] - 1;  // region with label 1 in M 
