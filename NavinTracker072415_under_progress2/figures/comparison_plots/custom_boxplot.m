function h=custom_boxplot(varargin)
...
%   $Revision: 1.1.8.8 $  $Date: 2011/05/09 01:24:13 $
...
g = [];
notch = 'off';
symbol = ''; % Navin 'plotstyledefault';
orientation = 'vertical';
whisker = 1.5;
labels = {};
labelverbosity = 'plotstyledefault';
colors = [0.5 0.5 0.5]; % Navin [];
colorgroup = [];
widths = [];
grouporder = [];
positions = [];
...
boxidx.mean = nanmean(xSorted1group);
...
list1 = addlistener(f,'SizeChange',@(src,evt) repositionLabels(ax));
...
switch(boxstyle)
    ...
    case 'outline',
        % Draw filled-boxstyle whiskers. % modified by Navin 7/10/13
        hwhisker = ...
            boxrenderer(boxparent,gPos,maxGuaranteedGap,orientation, ...  
            'lineAlongResponse',{...
                'locationstart',boxValPlot.wlo,...
                'locationend',boxValPlot.whi,...
                'linestyle','-','linewidth',1,'linecolor','k',...
                'tag','Whisker'}...
            );
        ...
        switch notch
            ...
            hbox =       boxrenderer(boxparent,gPos,gWidth,orientation, ...
                    'lineBox',{...
                        'locationstart',boxValPlot.q1,...
                        'locationend',boxValPlot.q3,...
                        'linestyle','-','linewidth',1,'linecolor',cBox,...
                        'tag','Box'}...
                );  
...
%                 'locationstart',boxValPlot.q1,...
%                 'locationend',boxValPlot.q3,...
%                 'linestyle','-','linewidth',0.01,'linecolor',cBox,...
%                 'tag','Box'}...
%             );
...
%    case 'line',
                     ...
                     hmedian = ...
            boxrenderer(boxparent,gPos,gWidth,orientation, ...   
            'lineAlongFactor',{'location',boxValPlot.mean,...
...
if selectedComponent<=7
    ...
    switch selectedComponent
        case 1  %'Lower Adjacent'
            datatipTxt = {sprintf('Lower Adjacent: %s',num2str(groupInfo.wlo))};
        case 2  %'25th Percentile'
            datatipTxt = {sprintf('25th Percentile: %s',num2str(groupInfo.q1))};
        case 3  % 'Lo Notch'
            datatipTxt = {sprintf('Lo Notch: %s',num2str(groupInfo.nlo))};
        case 4  % 'Median'
            datatipTxt = {sprintf('Median: %s',num2str(groupInfo.q2))};
        case 5  % 'Hi Notch'
            datatipTxt = {sprintf('Hi Notch: %s',num2str(groupInfo.nhi))};
        case 6  % '75th Percentile'
            datatipTxt = {sprintf('75th Percentile: %s',num2str(groupInfo.q3))};
        case 7  % 'Upper Adjacent'
            datatipTxt = {sprintf('Upper Adjacent: %s',num2str(groupInfo.whi))};
        otherwise
            datatipTxt = '';
            return; % Error.
    end
    ...
    datatipTxt = {datatipTxt{:},...
        [''],...
        sprintf('Group: %s',groupLabels),...
        [''],...
        sprintf('Maximum: %s',num2str(groupInfo.maximum)),...
        sprintf('Minimum: %s',num2str(groupInfo.minimum)),...
        sprintf('Num Points: %s',num2str(groupInfo.numPts)),...
        sprintf('Num Finite Outliers: %s',num2str(numFiniteOutliers)),...
        sprintf('Num NaN''s or Inf''s: %s',num2str(numNansAndInfs)),...
        };
    ...
    datatipTxt = {
        sprintf('Outlier Value: %s',num2str(cursorValue)),...
        [''],...
        };
    ...
    if length(cursorRows)==1
        ...
        datatipTxt = {datatipTxt{:},...
            sprintf('Observation Row: %s',num2str(cursorRows)),...
            };
    elseif length(cursorRows)<=handful
        ...
        cursorRowsChar(end) = [];
        datatipTxt = {datatipTxt{:},...
            sprintf('Observation Rows: %s',cursorRowsChar),...
            sprintf('Num Outliers At This Value: %s',num2str(length(cursorRows))),...
            };
    else
    ...
    datatipTxt = {datatipTxt{:},...
        sprintf('Group: %s',groupLabels),...
        [''],...
        sprintf('Distance To Median: %s',num2str(distToMedian)),...
        sprintf('Num IQRs To Median: %s',num2str(numIqrsToMedian)),...
        };
...
%   $Revision: 1.1.8.3 $  $Date: 2011/05/09 01:28:30 $
...
for subfuncindex = 1:numsubfuncs
    ...
    if ~ismember(subfunc, ...
        ...
        % %%%%%%%%%%%%%%%%%%%%%%%
        error(message('stats:boxrenderer:badSubFuncHandle'));
    end
...
if iscell(markertype) || iscell(markersize) || iscell(markercolor) || ...
        iscell(markerfill)
    error(message('stats:boxrenderer:TooManyLineStylesPerGroup'));
end
...
for currentGroup = 1:numgroups
    ...
    switch(grpMarkerfill)
        ...
        otherwise,
            error(message('stats:boxrenderer:BadMarkerFill'));
...
if iscell(lineStyle) || iscell(lineWidth) || iscell(lineColor)
    error(message('stats:boxrenderer:BadWidthColorStyle'));
end
...
for currentGroup = 1:numgroups
  ...
  h(currentGroup) =  patch(x,y,grpLineColor,'edgecolor','k','LineWidth',grpLineWidth); % Navin
   set(h(currentGroup),'tag',tag,'Parent',hgg);
%  hold on
%     h(currentGroup) = line(x,y, ...
%         'Marker','none',...
%         'Color','k',...
...
if iscell(colorInputArg)
    ...
    if size(groupColor,1)~=3
        error(message('stats:boxrenderer:ColorMustBeLength3Vectors'));
    end
elseif ischar(colorInputArg)
    ...
    elseif isvector(colorInputArg)
        ...
        error(message('stats:boxrenderer:unexpectedDataType'));
    end
else
    ...
    if ismember(1,sz)
        ...
        if sz(2)~=3
            error(message('stats:boxrenderer:MultipleColorsMatrixShape'));
        end
        groupColor = colorInputArg(groupindex,:);
    end
end
...
if any(cellfun(@iscell,varargin))
    ...
    if length(unique(vectlens))>2
        error(message('stats:boxrenderer:vectLenMismatchWithinGroup'));
    end
    %vectlen = max(vectlens);
end

%unpack data for each group, return as a vector for each group
varargout = cell(1,numvars);
for i=1:numvars
    ...
    elseif isvector(currentvar)
        ...
        error(message('stats:boxrenderer:InternalError'));
    end
end
...
if ischar(argValues{1}{1})
    ...
    [varargout{:}] = ...
        internal.stats.parseArgs(argNames,defaults,argValues{1}{:});
else
