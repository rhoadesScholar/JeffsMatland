function h = plot_sample_control_bar_line_histograms(sample, ctrl, bincenters, sample_color, ctrl_color, ylimits, xlabel_flag,fontclass,fontsize)
% h = plot_sample_control_bar_line_histograms(sample, ctrl, bincenters, sample_color, ctrl_color, [max_sample_yaxis max_ctrl_yaxis], xlabel_flag,fontclass,fontsize)

if(nargin<1)
    disp('h = plot_sample_control_bar_line_histograms(sample, ctrl, bincenters, sample_color, ctrl_color, [max_sample_yaxis max_ctrl_yaxis], xlabel_flag,fontclass,fontsize)')
    return;
end
...
h.top.plot = bar(100*y/sum(y),1,'FaceColor', sample_color,'edgecolor',sample_color);
...
if(~isempty(ctrl))
    ...
    hold on
    h.bottom.plot = plot(1:length(bincenters), 100*y/sum(y), 'o-','color',ctrl_color, 'markerfacecolor',ctrl_color,'linewidth',2,'markersize',2.5);
