function h_out = colorbarscale(varargin)
%COLORBARSCALE Color scale bar using part of figure's colormap
%
%   COLORBARSCALE creates a color scale for use with an integer-valued
...
%   standard COLORBAR function, COLORBARSCALE can limit the displayed
%   colors to the appropriate interval within the figure's colormap.
%
%   H = COLORBARSCALE(CMAPLIM,DATALIM,DATAINC,ORIENTATION,'Position',RECT)
...
%   axes containing the color scale).  Unlike COLORBAR, COLORBARSCALE does
%   not automatically decide where to place the axes.  COLORBARSCALE
%   optionally returns the axes handle.
%
%   H = COLORBARSCALE(...,PROP1,VAL1,PROP2,VAL2,...) sets additional
