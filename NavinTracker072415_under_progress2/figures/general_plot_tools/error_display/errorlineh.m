function h = errorlineh(varargin)
...
if(nargin>=5)
    h = plot(yb,xb,esymbol,'parent',cax,varargin{5:end}); hold(cax,'on')
    h = [h;plot(y,x,symbol,'parent',cax,varargin{5:end})]; 
else
    h = plot(yb,xb,esymbol,'parent',cax,'LineWidth',1.5); hold(cax,'on')
    h = [h;plot(y,x,symbol,'parent',cax,'LineWidth',1.5)]; 
end
