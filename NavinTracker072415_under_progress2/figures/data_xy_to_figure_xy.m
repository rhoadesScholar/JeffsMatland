function varargout = data_xy_to_figure_xy(varargin)
% data_xy_to_figure_xy -- Transform point or position from data space 
...
%
% Syntax:
%    [figx figy] = data_xy_to_figure_xy([x1 y1],[x2 y2])  % GCA is used
%    figpos      = data_xy_to_figure_xy([x1 y1 width height])
%    [figx figy] = data_xy_to_figure_xy(axes_handle, [x1 y1],[x2 y2])
%    figpos      = data_xy_to_figure_xy(axes_handle, [x1 y1 width height])
%
...
%   [figx figy] = data_xy_to_figure_xy(gca, axx, axy);  (now in figure space)
