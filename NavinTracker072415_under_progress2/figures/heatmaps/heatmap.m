function [hImage, hText, hTick, h_cB] = heatmap(mat, xlab, ylab, textmat, varargin)
...
%   lowest value in the colormap. To override that behavior and use a
%   different NaNColor, you must also set the 'UseFigureColormap' option to
%   false.
%
...
%   the minimum value of the matrix input. This only works if
%   'UseFigureColormap' is set to false. 
%
...
%   the maximum value of the matrix input. This only works if
%   'UseFigureColormap' is set to false. 
% 
...
%
% * 'TickInterpretTex': Set to 1 or true to render tick labels using a TEX
%   interpreter. For example, '_b' and '^o' would be rendered as subscript
%   b and the degree symbol with the TEX interpreter. (Default: false)
%
% OUTPUTS:
...
if isnan(param.MinColorValue)
    param.MinColorValue = min(mat(:));
end
if isnan(param.MaxColorValue)
    param.MaxColorValue = max(mat(:));
end

if(~isempty(param.caxis))
    param.MinColorValue = param.caxis(1);
    param.MaxColorValue = param.caxis(2);
end
...
if ~all(isnan(param.NaNColor))
    param.UseFigureColormap = false;
end
...
% Add color bar
function h_cB = addColorbar(p, mat, textmat)
...
originalticks = get(c,'YTick');
if isempty(originalticks)
    originalticks = get(c,'XTick');
    tickAxis = 'X';
else
    tickAxis = 'Y';
end
...
if ~p.UseFigureColormap
    ...
    if x == n, x = n+1; end
    ticks = n:(x-n)/8:x;
    set(c,'YTick',[1:(p.ColorLevels-1)/8:p.ColorLevels]);
end
if ~isempty(ticks)
    ...
    if ischar(textmat) % If format string, format colorbar ticks in the same way
        ...
        ticklabels = arrayfun(@(x){sprintf('%.2f',x)},ticks); % num2str(ticks(:));
    end
...
% Set axes labels
function [p, xlab, ylab, hTick] = setAxesLabels(p, xlab, ylab)
...
if isempty(xlab)
    ...
    if p.TickAngle == 0
        ...
        hTick = createXTicks(p.hAxes, p.TickAngle, xtick, xlab(xtick), p.TickTexInterpreter);
        adjustAxesToAccommodateTickLabels(p.hAxes, hTick);
    end
end
...
function hTick = createXTicks(hAxes, tickAngle, xticks, xticklabels, texInterpreter)
    ...
    hTick = text(xPos, yPos, cellstr(xticklabels), 'Units', 'normalized', ...
...
function adjustAxesToAccommodateTickLabels(hAxes, hTick)
...
[axPosP, axPosN, axOPP, axOPN, coPosP, textPosP]  = ...
    getGraphicsObjectsPositions(hAxes, hTick); %#ok<ASGLU>
...
function [axPosP, axPosN, axOPP, axOPN, coPosP, textPosP, textPosN] =...
         getGraphicsObjectsPositions(hAxes, hTick)
...
set(hTick,'Units','pixels'); % Measure height in pixels
extents = get(hTick,'Extent'); % Get heights for all text objects
...
set(hTick,'Units','normalized'); % Restore previous behavior
extents = get(hTick,'Extent'); % Get heights for all text objects
extents = vertcat(extents{:}); % Collect heights in one matrix
...
if ~isempty(axInfo.xlab)    
    ...
    if ~isempty(axInfo.hTick) % Rotated X tick labels exist
        try delete(axInfo.hTick); end %#ok<TRYNC>
        axInfo.hTick = createXTicks(hAxes, p.TickAngle, xticks, xlabels(xticks), p.TickTexInterpreter);
        set(hAxes, 'UserData', axInfo);
    else
        set(hAxes, 'XTick', xticks, 'XTickLabel', xlabels(xticks));
    end
    
    %adjustAxesToAccommodateTickLabels(hAxes, axInfo.hTick)
end
...
if isscalar(textmat) && textmat % If true convert mat to text
    displaytext = arrayfun(@(x){num2str_nanspace(x)},mat);
elseif ischar(textmat) % If a format string, convert mat to text with specific format
    displaytext = arrayfun(@(x){sprintf(textmat,return_nonnan(x))},mat);
elseif isnumeric(textmat) && numel(textmat)==numel(mat) % If numeric, convert to text
    displaytext = arrayfun(@(x){num2str_nanspace(x)},textmat);
elseif iscell(textmat) && numel(textmat)==numel(mat) % If cell array of strings, it is already formatted
    displaytext = textmat;
else
