...
tempfilename = sprintf('%s%szwspMIxnyn.txt',tempdir,filesep);
save(tempfilename, 'zwsp', '-ASCII')

% save zwspMIxnyn.txt zwsp -ASCII

exec_filename = sprintf('%s%sMIxnyn.exe',fileparts(which('MIxnyn')),filesep);

% execute C Program
dir = pwd;
cd(tempdir);
cp(exec_filename,sprintf('%s%sMIxnyn.exe',tempdir,filesep));

[a unout]=system(['MIxnyn.exe zwspMIxnyn.txt ',num2str(Ndx),' ',num2str(Ndy),' ',num2str(N),' ',num2str(kneig)]);
miout=str2num(unout);
rm(tempfilename);
cd(pwd);
