...
%   $Revision: 1.21.4.12 $  $Date: 2011/04/16 06:39:32 $
...
if nIn < 2
    error(message('MATLAB:INTERSECT:NotEnoughInputs'));
end
...
if ~(isa(a,'opaque') || isa(b,'opaque')) 
    ...
    if isempty(flag)
        ...
        if length(a)~=numelA || length(b)~=numelB
            error(message('MATLAB:INTERSECT:AandBvectorsOrRowflag'));
        end
        ...
        if ~isrows
            error(message('MATLAB:INTERSECT:UnknownFlag'));
        end
