function [ind,t0,s0,t0close,s0close] = zero_crossing(S,t,level,imeth)
% ZERO_CROSSING find the crossings of a given level of a signal
%   ind = ZERO_CROSSING(S) returns an index vector ind, the signal
%   S crosses zero at ind or at between ind and ind+1
%   [ind,t0] = ZERO_CROSSING(S,t) additionally returns a time
%   vector t0 of the zero crossings of the signal S. The crossing
%   times are linearly interpolated between the given times t
%   [ind,t0] = ZERO_CROSSING(S,t,level) returns the crossings of the
%   given level instead of the zero crossings
%   ind = ZERO_CROSSING(S,[],level) as above but without time interpolation
%   [ind,t0] = ZERO_CROSSING(S,t,level,par) allows additional parameters
%   par = {'none'|'linear'}.
