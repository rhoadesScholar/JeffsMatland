...
%
%   See also SHOWFIT, SELECTFIT, EZFIT, SHOWEQBOX, DISPEQFIT, PICKDATA.
...
%           selection of the data is fitted (see selectfit).
...
% Open the Array Editor with the fit coefficients after showfit or
% selectfit: 'on' or 'off'. See Editcoeff for details.
...
% Loop mode for SelectFit:
%  'on': the polygon procedure is repeated until an empty polygon is drawn
%  'off': the polygon procedure is done once.
fp.selectfitloopmode = 'off';


% Message box display for SelectFit: 'on' or 'off'.
fp.selectfitmsgbox = 'on';


% Markers used for the selected data with selectfit:
% (set to '' if you don't want the selected points to be displayed)
fp.markerselectpt = '*';
