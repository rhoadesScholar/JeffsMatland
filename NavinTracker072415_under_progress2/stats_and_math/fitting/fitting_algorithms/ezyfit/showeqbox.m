...
%      showeqbox(f);
%
%   See also FITPARAM, EZFIT, SHOWFIT, SELECTFIT, DISPEQFIT.

%   F. Moisy, moisy_at_fast.u-psud.fr
%   Revision: 1.12,  Date: 2006/10/18
...
if isfield(f,'eq'), % for normal fits (no interpolation):
    streq = fix_title_string([f.yvar '(' f.xvar ') = ' f.eq]);  
    ...
    if strcmp(fp.eqreplacemode,'on'),
        ...
        for n=1:length(f.m)
            if(~isfield(f,'m_error'))
                ...
                strm = [greekize(f.param{n}) ' = ' num2str(f.m(n), fp.numberofdigit) ' +/- ' num2str(f.m_error(n), fp.numberofdigit)];
            end
    ...
    if(isfield(f,'chi2'))
        lastline=[fix_title_string('chi2 = ') num2str(f.chi2, fp.numberofdigit) '  '];
        streq = {streq{:} lastline};
    end
    
    if(isfield(f,'aic'))
        lastline=[fix_title_string('aic = ') num2str(f.aic, fp.numberofdigit) '  '];
        streq = {streq{:} lastline};
    end
...
% position of the new annotation textbox, to avoid overlaping:
...
hboxeq=annotation('textbox',position,'BackgroundColor',bgcolor,...
    'Margin',3,'Color',fitcolor,'EdgeColor',fitcolor,...
    'FitHeightToText','on','FitHeightToText','on',...
    'UserData','equationbox','String',streq);
