...
% edited by Navin to include standard error for fitted parameters based on 
% jean-luc.dellis@u-picardie.fr  easyfit.m from Matlab Central
...
%   fit. Use SHOWFIT to display the fit. In order to fit only part of the
%   data, use SHOWFIT with the "Data Brushing" mode  (Matlab >=7.6), or
%   SELECTFIT (Matlab <=7.5).
%
...
if (strcmp(f.fitmode,'log') && nansum(y<=0)>0)
...
if ~exist('m0','var')
    ...
    switch usepredefinedfit   % new v2.50 : "magic" initial guesses
        ...
        case 'affine'   % a*x+b
            ...
            m0(2) = nanmean(y-m0(1)*x);              % b
        case 'affineshift'   % a*(x-b)
            m0(1) = (y(end)-y(1))/(x(end)-x(1));  % a
            m0(2) = nanmean(x-y/m0(1));              % b
        case 'power'    % a*x^n;
            % if both x and y are of constant sign:
            if nansum(diff(sign(x)))==0 || nansum(diff(sign(y)))==0
                m0(2) = log(y(end)/y(1)) / log(x(end)/x(1));  % n
                m0(1) = nanmean(y./(x.^m0(2)));                  % a
            end
        case 'powerc'    % a*x^n+c;
            % if both x and y are of constant sign:
            if nansum(diff(sign(x)))==0 || nansum(diff(sign(y)))==0
                m0(3) = log(y(end)/y(1)) / log(x(end)/x(1));  % n
                m0(1) = nanmean(y./(x.^m0(3)));                  % a
            end
            m0(2) = nanmean(y);  % c
        case 'powershift'    % a*(x+b)^n;
            ...
            m0(1) = nanmean(y./((x+m0(2)).^m0(3)));                  % a           
        case 'exp'    % a*exp(b*x)
            m0(2) = (log(y(end)/y(1))) / (x(end)-x(1));  % b
            m0(1) = nanmean(y./exp(m0(2)*x));               % a
        case 'expdiv'    % a*exp(x/b)
            m0(2) = (x(end)-x(1)) / (log(y(end)/y(1)));  % b
            m0(1) = nanmean(y./exp(x/m0(2)));               % a
        case 'explim'    % a*(1-exp(-x/b))
            m0(1) = max(y);                              % a
            m0(2) = nanmean(-x./(log(1-y/m0(1))));          % b
        case 'expc'    % a*exp(b*x)+c
            m0(3) = nanmean(y);                             % c
            m0(2) = (log((y(end)-m0(3))/(y(1)-m0(3)))) / (x(end)-x(1));  % b
            m0(1) = nanmean((y-m0(3))./exp(m0(2)*x));               % a
        case 'log'    % a*log(b*x)
            m0(1) = (y(end)-y(1))/(log(x(end)/x(1)));    % a
            m0(2) = nanmean(exp(y/m0(1))./x);               % b            
        case 'logc'    % a*log(x)+b
            m0(1) = (y(end)-y(1))/(log(x(end)/x(1)));    % a
            m0(2) = nanmean(y - m0(1)*log(x));              % b     
        ...
        case 'sinphic'    % a*sin(b*x+phi)+c
            ...
            m0(4) = 1;                 % phi
        case 'cngauss'
            m0(1) = (nanmean(x.^2.*y)/nanmean(y))^(1/2);   % sigma
        case 'cfgauss'   % a*exp(-(x^2)/(2*sigma^2))
            m0(1) = max(y);      % a
            m0(2) = (nanmean(x.^2.*y)/nanmean(y))^(1/2);   % sigma
        case 'ngauss'    % exp(-((x-x_0)^2)/(2*sigma^2))/(2*pi*sigma^2)^(1/2)           
            m0(2) = nanmean(x.*y)/nanmean(y);             % x_0
            m0(1) = (nanmean((x-m0(2)).^2.*y)/nanmean(y))^(1/2);  % sigma
        case {'fgauss','gauss'}    % a*exp(-((x-x_0)^2)/(2*sigma^2))
            m0(1) = max(y);     % a
            m0(3) = nanmean(x.*y)/nanmean(y);             % x_0                   
            m0(2) = (nanmean((x-m0(3)).^2.*y)/nanmean(y))^(1/2);  % sigma
    end
...
% fitting itself
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
mfe = 10000*length(m0); % added by Navin
options = optimset('MaxFunEvals',mfe,'MaxIter',mfe);
...
switch f.fitmode,
    case 'lin',
        ...
        y_fit = eval(eqml);
        ssr = nansum(abs(y_fit-nanmean(y)).^2);
        sse = nansum(abs(y_fit-y).^2);
        f.r2=(ssr/(sse+ssr));
        f.chi2 = fitlin(m);
    case 'log',
        x__ref = x;
        try
            m=fminsearch(@fitlog, m0,options);
        catch
            error('Ezyfit:ezfit:fminsearchError','Fit: error during the fminsearch procedure');
        end
        y_fit = eval(eqml);
        ssr = nansum(abs(log(y_fit)-nanmean(log(y))).^2);
        sse = nansum(abs(log(y_fit)-log(y)).^2);
        f.r2=(ssr/(sse+ssr));
        f.chi2 = fitlog(m);
    otherwise
...
% fitting errors - Navin
[jac,r] = jacob(x__ref,y,eqml,m);
m_error = zeros(size(m));
delta_y = zeros(size(y_fit));
[m_error,delta_y] = uncertainties(m,r,jac,x,y_fit);
f.df = length(delta_y) - length(m) -  1; % degrees of freedom
...
f.param = param;
f.m = m;
f.m_error = m_error; % standard error of fitted parameters
f.m0 = m0;
f.x = x;
f.y = y;
f.yfit = evalfit(f);
f.d_95_y = delta_y;  % 95% confidence interval for y
[f.aic, f.ss] = aic_from_fit(f);
...
% curve and the fit), in lin or log mode:
    ...
    function chi2 = fitlin(m)
        y_fit = eval(eqml);
        chi2 = nansum( ((y_fit - y).^2) ); % sum( ((y_fit - y).^2)./(dy.^2) ); 
    end

    function chi2 = fitlog(m)
        y_fit = eval(eqml);
        chi2 = nansum((log(y_fit)-log(y)).^2);
    end
end
...
diag_info = nansum((Rinv.*Rinv)')';
v = m-n;                        % number of degrees of liberty
rmse = sqrt(nansum(r.*r)/v);
E = jac*Rinv;
...
function [jac,r] = jacob(x__ref,y,eqml,parameters)
% 
...
zerosp = zeros(p,1);
    for k = 1:p
        ...
        m = parameters+delta; yplus = eval(eqml); % yplus = fhandle(parameters+delta,x__ref);
        dy = yplus(:) - yfit(:);
        jac(:,k) = dy/delta(k);
    end
