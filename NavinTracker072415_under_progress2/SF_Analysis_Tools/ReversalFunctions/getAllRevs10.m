...
%%%%%% DWELLING STATE AND CALCULATES THE SPEED FOR 10 SECONDS AFTER THE
...
function ReversalInfo = getAllRevs10(folder1)
...
%%%%%%%%%%(1)Dwell/Roam call (2) trackNumb and (3) EndofRev
    ...
    [RevMatrix DwellRevRate RoamRevRate Dwell_sRevRate Dwell_lRevRate Roam_sRevRate Roam_lRevRate] = CreateRevMatrix2(allTracks);
...
for (i = 1:(length(allTracks)))
    ...
    for (k = 1:NumRevs)
        ...
        TenSecAfterPlusOne = (EndofRev+2):(EndofRev+32);
        display(RevMatrix(k,:))
        if(TenSecAfterPlusOne(30) <= allTracks(trackNumb).NumFrames)
            %%%adjust for Framestart
            if (allTracks(trackNumb).Reversals(1,1) < allTracks(trackNumb).Frames(1))
            else
              TenSecAfterPlusOne = TenSecAfterPlusOne - allTracks(trackNumb).Frames(1) +1;
           end
            %FirstFrame = allTracks(trackNumb).Frames(1);
            %display(FirstFrame)
            %ThreeSecAfterPlusOne = ThreeSecAfterPlusOne - FirstFrame + 1;
            %display(TenSecAfterPlusOne)
            AngSpeedPostRev = allTracks(trackNumb).AngSpeed(TenSecAfterPlusOne);
            ...
            if(allnans == 0)
                if (check1>0)
                    ...
                    TenSecAfter = (EndofRev+2):(EndofRev+31);
                    
                    if (allTracks(trackNumb).Reversals(1,1) < allTracks(trackNumb).Frames(1))
                    else
                      TenSecAfter = TenSecAfter - allTracks(trackNumb).Frames(1) +1;
                   end
                            %%%adjust for Framestart

                    %ThreeSecAfter = ThreeSecAfter - FirstFrame + 1;
                    SpeedPostRev =  allTracks(trackNumb).Speed(TenSecAfter);
                    AngSpeedPostRev =  allTracks(trackNumb).AngSpeed(TenSecAfter);
                    if (RevMatrix(k,1) == 1)
                        DwellSpeedMatrix(indexDwell,1:30) = SpeedPostRev;
                        DwellAngSpeedMatrix(indexDwell,1:30) = AngSpeedPostRev;
                        indexDwell = indexDwell + 1;
                    else
                        RoamSpeedMatrix(indexRoam,1:30) = SpeedPostRev;
                        RoamAngSpeedMatrix(indexRoam,1:30) = AngSpeedPostRev;
                        indexRoam = indexRoam + 1;
                    end
                    
                    if (RevMatrix(k,4) < 0.3)
                        if (RevMatrix(k,1) == 1)
                            sRevDwellSpeedMatrix(indexDwellsRev,1:30) = SpeedPostRev;
                            indexDwellsRev = indexDwellsRev + 1;
                        end
                    else
                        if (RevMatrix(k,1) == 1)
                            lRevDwellSpeedMatrix(indexDwelllRev,1:30) = SpeedPostRev;
                    ...
                    if (RevMatrix(k,4) < 0.3)
                        if (RevMatrix(k,1) == 2)
                            sRevRoamSpeedMatrix(indexRoamsRev,1:30) = SpeedPostRev;
                            indexRoamsRev = indexRoamsRev + 1;
                        end
                    else
                        if (RevMatrix(k,1) == 2)
                            lRevRoamSpeedMatrix(indexRoamlRev,1:30) = SpeedPostRev;
    ...
    ReversalInfo = struct('DwellSpeedMatrix',[],'RoamSpeedMatrix',[],'sRevDwellSpeedMatrix',[],'lRevDwellSpeedMatrix',[],'sRevRoamSpeedMatrix',[],'lRevRoamSpeedMatrix',[],'DwellAngSpeedMatrix',[],'RoamAngSpeedMatrix',[],'DwellRevRate',[],'RoamRevRate',[],'Dwell_sRevRate',[],'Dwell_lRevRate',[],'Roam_sRevRate',[],'Roam_lRevRate',[])
    ...
    for (i=1:30) dwellspeedmean(i) = nanmean(DwellSpeedMatrix(:,i)); end;
    end
    
    if (length(RoamSpeedMatrix)>1)
    for (i=1:30) roamspeedmean(i) = nanmean(RoamSpeedMatrix(:,i)); end;
    end
    
    if (length(sRevDwellSpeedMatrix)>1)
    for (i=1:30) srevdwellspeedmean(i) = nanmean(sRevDwellSpeedMatrix(:,i)); end;
    end
    
    if (length(lRevDwellSpeedMatrix)>1)
    for (i=1:30) lrevdwellspeedmean(i) = nanmean(lRevDwellSpeedMatrix(:,i)); end;
    end
    
    if (length(sRevRoamSpeedMatrix)>1)
    for (i=1:30) srevroamspeedmean(i) = nanmean(sRevRoamSpeedMatrix(:,i)); end;
    end
    
    if (length(lRevRoamSpeedMatrix)>1)
    for (i=1:30) lrevroamspeedmean(i) = nanmean(lRevRoamSpeedMatrix(:,i)); end;
    end
    ...
    for(i=1:30) dwellangspeedmean(i) = mean(DwellAngSpeedMatrix(:,i)); end;
    end
    
    if (length(RoamAngSpeedMatrix)>1)
    for(i=1:30) roamangspeedmean(i) = mean(RoamAngSpeedMatrix(:,i)); end;
    end
