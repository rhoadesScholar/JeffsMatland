...
function ReversalInfo = getAllRevs_HMM_useN2HMM(folder1,N2_TR,N2_E)
...
%%%%%%%%%%(1)Dwell/Roam call (2) trackNumb and (3) EndofRev
    ...
    [RevMatrix DwellRevRate RoamRevRate Dwell_sRevRate Dwell_lRevRate Roam_sRevRate Roam_lRevRate] = CreateRevMatrix2_HMM_useN2HMM(allTracks,N2_TR,N2_E);
...
for (i = 1:(length(allTracks)))
    ...
    for (k = 1:NumRevs)
        ...
        ThreeSecAfterPlusOne = (EndofRev+2):(EndofRev+8);
        display(RevMatrix(k,:))
        if(ThreeSecAfterPlusOne(7) <= allTracks(trackNumb).NumFrames)
            ...
            if(allnans == 0)
                if (check1>0)
                    ...
                    ThreeSecAfter = (EndofRev+2):(EndofRev+7);
                    %%%adjust for Framestart
                    ...
                    AllRevsSpeedMatrix(indexAll,1) = RevMatrix(k,1);
                    AllRevsSpeedMatrix(indexAll,2) = RevMatrix(k,2);
                    AllRevsSpeedMatrix(indexAll,3) = RevMatrix(k,3);
                    AllRevsSpeedMatrix(indexAll,4) = nanmean(SpeedPostRev(4:6));
                    indexAll = indexAll+1;
                    if (RevMatrix(k,1) == 1)
                        DwellSpeedMatrix(indexDwell,1:6) = SpeedPostRev;
                        DwellAngSpeedMatrix(indexDwell,1:6) = AngSpeedPostRev;
                        indexDwell = indexDwell + 1;
                    else
                        RoamSpeedMatrix(indexRoam,1:6) = SpeedPostRev;
                        RoamAngSpeedMatrix(indexRoam,1:6) = AngSpeedPostRev;
                        indexRoam = indexRoam + 1;
                    end
                    
                    if (RevMatrix(k,4) < 0.3)
                        if (RevMatrix(k,1) == 1)
                            sRevDwellSpeedMatrix(indexDwellsRev,1:6) = SpeedPostRev;
                            indexDwellsRev = indexDwellsRev + 1;
                        end
                    else
                        if (RevMatrix(k,1) == 1)
                            lRevDwellSpeedMatrix(indexDwelllRev,1:6) = SpeedPostRev;
                    ...
                    if (RevMatrix(k,4) < 0.3)
                        if (RevMatrix(k,1) == 2)
                            sRevRoamSpeedMatrix(indexRoamsRev,1:6) = SpeedPostRev;
                            indexRoamsRev = indexRoamsRev + 1;
                        end
                    else
                        if (RevMatrix(k,1) == 2)
                            lRevRoamSpeedMatrix(indexRoamlRev,1:6) = SpeedPostRev;
    ...
    ReversalInfo.AllRevsSpeedMatrix = AllRevsSpeedMatrix;
    dwellspeedmean =[];
    roamspeedmean= [];
    srevdwellspeedmean = [];
    lrevdwellspeedmean = [];
    ...
    for (i=1:6) dwellspeedmean(i) = nanmean(DwellSpeedMatrix(:,i)); end;
    end
    
    if (length(RoamSpeedMatrix)>1)
    for (i=1:6) roamspeedmean(i) = nanmean(RoamSpeedMatrix(:,i)); end;
    end
    
    if (length(sRevDwellSpeedMatrix)>1)
    for (i=1:6) srevdwellspeedmean(i) = nanmean(sRevDwellSpeedMatrix(:,i)); end;
    end
    
    if (length(lRevDwellSpeedMatrix)>1)
    for (i=1:6) lrevdwellspeedmean(i) = nanmean(lRevDwellSpeedMatrix(:,i)); end;
    end
    
    if (length(sRevRoamSpeedMatrix)>1)
    for (i=1:6) srevroamspeedmean(i) = nanmean(sRevRoamSpeedMatrix(:,i)); end;
    end
    
    if (length(lRevRoamSpeedMatrix)>1)
    for (i=1:6) lrevroamspeedmean(i) = nanmean(lRevRoamSpeedMatrix(:,i)); end;
    end
    ...
    for(i=1:6) dwellangspeedmean(i) = mean(DwellAngSpeedMatrix(:,i)); end;
    end
    
    if (length(RoamAngSpeedMatrix)>1)
    for(i=1:6) roamangspeedmean(i) = mean(RoamAngSpeedMatrix(:,i)); end;
    end
