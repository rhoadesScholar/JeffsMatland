...
sqSizeOld=2; %squareSize for re-tracking on old positions
sqSize=3; %squareSize for re-tracking with maximum detection on interpolated subRegion of image
...
%use threshold from second frame (first often empty)
initialTrshOld=dat(2,14);

%PkCurrSm=[41 41];
    
D=dir([pn,'*.tif']);

%start on second frame bc first is often empty
