function [cellData FinalStretchTable ForwardRunStarts AllSmoothData] = ProcessCalciumImaging_NormFo_JLProc(FileName,NSMflag,StretchLength,PositiveDataCutoff,FluorChangeCutoff,beginMin,stopMin,AllSqIntOld_matlab)
...
%%%%Replace column 12 with Re-Tracked data from JL script 

for(j=1:length(cellData(:,1)))
    FrameHere = cellData(j,1);
    NormCaHere = AllSqIntOld_matlab(FrameHere);
    cellData(j,12) = NormCaHere;
end
...
%%%%%%%Convert fluorescence to a F/Fo scale = Column 23

beginFrame = (beginMin*600)-599;
endFrame = (stopMin*600);
FramesOfInt = beginFrame:endFrame;
[c, rowsOfInt, ib] = intersect(cellData(:,1),FramesOfInt);
AvgFluor_Fo = nanmean(cellData(rowsOfInt,12));

newCellData = cellData(:,12) / AvgFluor_Fo;
