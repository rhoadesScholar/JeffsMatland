function [dsd1_InclEnds rsd1_InclEnds dsd2_InclEnds rsd2_InclEnds] = JeffcompareTwoGenotypes_HMM_useN2HMM(N2Tracks, Genotype2, allTracks2, Date)
...
%     display(PathName)
   ...
   [dsd1 rsd1 FracDwell1 FracRoam1 TrackInfo1 N2_TR N2_E] = JeffAutomatedRoamDwellAnalysis_Pool_HMM_collectN2HMM(N2Tracks,Date,'N2');
   ...
   [dsd1_InclEnds rsd1_InclEnds FracDwell1_InclEnds FracRoam1_InclEnds N2_TR_InclEnds N2_E_InclEnds mean_dw_stab1 mean_dw_stab_err1 mean_dw_stab_vector1 mean_ro_stab1 mean_ro_stab_err1 mean_ro_stab_vector1  all_dw_speed_info all_ro_speed_info] = GetHistsAndRatioInclEnds_HMM_collectN2HMM(N2Tracks,Date,'N2');
...
%    PathName = sprintf('%s/%s/',PathofFolder,string2);
   ...
   [dsd2 rsd2 FracDwell2 FracRoam2 TrackInfo2] = JeffAutomatedRoamDwellAnalysis_Pool_HMM_useN2HMM(allTracks2,Date,Genotype2,N2_TR,N2_E);
   ...
   [dsd2_InclEnds rsd2_InclEnds FracDwell2_InclEnds FracRoam2_InclEnds mean_dw_stab2 mean_dw_stab_err2 mean_dw_stab_vector2 mean_ro_stab2 mean_ro_stab_err2 mean_ro_stab_vector2  all_dw_speed_info all_ro_speed_info] = GetHistsAndRatioInclEnds_HMM_useN2HMM(allTracks2,Date,Genotype2,N2_TR_InclEnds,N2_E_InclEnds);
   ...
   TrackInfo2.roam_state_durations_incl_ends = rsd2_InclEnds;
    ...
    VidName = sprintf('%s.%s',Date,Genotype2);
    %display(VidName)
    NewFilename = sprintf('%s.TrackInfo.mat',VidName);
    %display(NewFilename)
%     PathofFolderforSave = sprintf('%s',PathName);
%     FullFileName = sprintf('%s/%s',PathofFolderforSave,NewFilename);
   ...
   [rsd_mean_incl_ends2 rsd_err_incl_ends2 rsd_vector_incl_ends2] = AverageStateDur(TrackInfo2.roam_state_durations_incl_ends);
    ...
    set(gca,'XTickLabel',{'N2';Genotype2});
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if(size(TrackInfo2.dwell_state_durations)>0)
    subplot(4,4,2);
    
    plotTwoHists(dsd1(1,:),dsd2(1,:),'N2',Genotype2,10);
    title('Dwell States');
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if(size(TrackInfo2.dwell_state_durations)>0)
    ...
    plotTwoHists(rsd1(1,:),rsd2(1,:),'N2',Genotype2,10);
    title('Roam States');
    ...
    plotTwoHists(dsd1_InclEnds(1,:),dsd2_InclEnds(1,:),'N2',Genotype2,10);
    ...
    plotTwoHists(rsd1_InclEnds(1,:),rsd2_InclEnds(1,:),'N2',Genotype2,10);
...
%     set(gca,'XTickLabel',{'all Revs';'';'';'sRevs';'';'';'lRevs';''});
    ...
    legend1 = sprintf('%s.Dwell','N2');
    legend2 = sprintf('%s.Dwell',Genotype2);
    legend3 = sprintf('%s.Roam','N2');
    legend4 = sprintf('%s.Roam',Genotype2);
    ...
    %%%%Save Figure
    CompName = sprintf('%s.%s.%s',Date,'N2',Genotype2);
    save_figure(gcf,'',CompName,'comparison');
end
