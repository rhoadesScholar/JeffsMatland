function showHMMStateCalls_SpecifyModel(finalTracks,vectToAnalyze,binSize,trans,emis)
    ...
    for(i=1:numTracks)
        ...
        %[stateList startingStateMap] = getStateSliding_Diff(finalTracks,finalTracks2,450,30,3,35,57,3);
           ...
           % title(dummystring);
            ...
            [expNewSeq expStates estTR estE] = getHMMStatesSpecifyTRandE_2(finalTracks,30,trans,emis);
            ...
            xaxis_7 = 1:1:length(expNewSeq(vectToAnalyze(i)).states);
            xaxis_8 = xaxis_7/180;
            subplot(4,1,4);
            plot(xaxis_8,expNewSeq(vectToAnalyze(i)).states);
            axis([0 90 -1 3]);
            dummystring = sprintf('Original Bins');
            title(dummystring);
            %%%maybe set color, etc.
        
        [filepath,fileprefix,extension] = fileparts(finalTracks(i).Name);
        display(fileprefix)
