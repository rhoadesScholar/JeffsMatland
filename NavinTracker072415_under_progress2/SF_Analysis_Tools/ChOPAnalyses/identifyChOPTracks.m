...
function ChOPfinalTracks = identifyChOPTracks(finalTracks,stimulusfile,ROImovie,plotthis,C128SFlag,stimtoIncl)
...
for (i=1:length(TracksInStim(:,1)))
    ...
    if(C128SFlag==1)
        ...
        finalTracks(TracksInStim(i,2)).stimulus_vector(((stimulus((currentStim-1),1)*3)-FirstFrame+1):((stimulus((currentStim-1),2)*3)-FirstFrame+1)) = 1;
        finalTracks(TracksInStim(i,2)).stimulus_vector(((stimulus(currentStim,1)*3)-FirstFrame+1):((stimulus(currentStim,2)*3)-FirstFrame+1)) = 2; 
        end
    else
    finalTracks(TracksInStim(i,2)).stimulus_vector(((stimulus(currentStim,1)*3)-FirstFrame+1):((stimulus(currentStim,2)*3)-FirstFrame+1)) = 1;    
    end
