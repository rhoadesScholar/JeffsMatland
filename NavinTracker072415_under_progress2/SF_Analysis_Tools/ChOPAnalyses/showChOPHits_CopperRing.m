function [SpeedTests AngSpeedTests TestAvgSpeed TestControlSpeed FullControlChOPHits] = showChOPHits_CopperRing(folder,stimulusfile,stimtoIncl,C128SFlag)
    
    stimulus = load(stimulusfile);
    if(stimtoIncl==0)
        stimtoIncl= 1:1:length(stimulus(:,1));
    end
    [AllChOPHits NumberofFOI] = summarizeChOPdata_Copper(folder,stimulusfile,C128SFlag,stimtoIncl);
