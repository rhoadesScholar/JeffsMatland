function [DwSpeedTest DwAngSpeedTest RoSpeedTest RoAngSpeedTest p_chi_Dw p_chi_Ro st_checkDw st_checkRo] = showChOPHits_CopperRing_comparePoolsToN2_useN2HMM(n2_folders,N2_stimulusfile,exp_folders,exp_stimulusfile,stimtoIncl,C128SFlag)
    FullControlChOPHits = [];
    AllChOPHits = [];
    ...
    for(i=1:(length(n2_folders)))
        AllChOPHitsTemp = [];
        [AllChOPHitsTemp NumberofFOI estTR estE] = summarizeChOPdata_Copper_collectHMM(n2_folders{i},N2_stimulusfile,C128SFlag,stimtoIncl_N2);
    ...
    for(i=1:(length(exp_folders)))
        ...
        AllChOPHitsTemp = [];
        [AllChOPHitsTemp NumberofFOI] = summarizeChOPdata_Copper_useHMM(exp_folders{i},exp_stimulusfile{i},C128SFlag,stimtoIncl_exp,estTR,estE);
...
%     end
    %hold on; errorshade((1:NumberofFOI)/3,FullControlupper,FullControllower,[0.95 0.95 0.95]);
    hold on; area((1:NumberofFOI)/3,RoStates);
