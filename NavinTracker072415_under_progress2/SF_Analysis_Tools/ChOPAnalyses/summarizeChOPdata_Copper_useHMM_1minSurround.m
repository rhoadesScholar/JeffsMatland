...
function [AllChOPHits AllChOPInfo_forBinnedData NumberofFOI] = summarizeChOPdata_Copper_useHMM_1minSurround(folder,stimulusfile,C128SFlag,stimtoIncl,estTR,estE)
    ...
    ChOPfinalTracks = identifyChOPTracks_Copper(allTracks,stimulusfile,0,C128SFlag,stimtoIncl);
   %%%%%%%%%%calculate HMM based on all data from all animals
    ...
    lengthofGreenFrames = ((stimulus(stimtoIncl(1)+1,2)-stimulus(stimtoIncl(1)+1,1)) * 3);
    for(i=1:length(ChOPfinalTracks))
        ...
        for(j=1:numStimuli)
            if(C128SFlag==1)
                StartIndex = (BlueFrames((lengthofStimFrames*j)-(lengthofStimFrames-1)))-180;
                StopIndex = StartIndex+179+lengthofStimFrames+lengthofGreenFrames-2+180;
            else
            ...
            StartFrame = ChOPfinalTracks(i).Frames(StartIndex+180);
            ...
            Prior30sData = StateCalls(91:180);
            NumRoam = length(find(Prior30sData==2));
            NumDwell= length(find(Prior30sData==1));
            Total = length(Prior30sData);
            if((NumRoam/Total) > .66)
                PriorState = 2;
            elseif ((NumDwell/Total) > .66)
                PriorState = 1;
            else
                PriorState = 3;
            end
            ...
            while(cntrlflag==0)
                ...
                ControlStartIndex = ControlStopIndex-179-lengthofStimFrames-lengthofGreenFrames+2-180;
            ...
            Prior30sData = ControlStateCalls(91:180);
            NumRoam = length(find(Prior30sData==2));
            NumDwell= length(find(Prior30sData==1));
            Total = length(Prior30sData);
            if((NumRoam/Total) > .66)
            ...
            while(fullcnt<11)
                ...
                FullControlStartIndex = FullControlStopIndex-179-lengthofStimFrames-lengthofGreenFrames+2-180;
            ...
            Prior30sData = FullControlStateCalls(91:180);
            NumRoam = length(find(Prior30sData==2));
            NumDwell= length(find(Prior30sData==1));
            Total = length(Prior30sData);
            if((NumRoam/Total) > .66)
                FullControlPriorState = 2;
            elseif ((NumDwell/Total) > .66)
                FullControlPriorState = 1;
            else
                FullControlPriorState = 3;
            end
             
             
             
             %FullControlPriorState = round(nanmean(FullControlStateCalls(271:360)));
             
             if(FullControlPriorState==PriorState)
                 FullControlPriorData(fullcnt) = FullControlPriorState;
                 if(C128SFlag==1)
                    FullControlResultingState(fullcnt) = round(nanmean(FullControlStateCalls((180+lengthofStimFrames+lengthofGreenFrames):((180+lengthofStimFrames+lengthofGreenFrames)+89))));
                    else
            ...
            if(C128SFlag==1)
                ResultingState = round(nanmean(StateCalls((180+lengthofStimFrames+lengthofGreenFrames):((180+lengthofStimFrames+lengthofGreenFrames)+89))));
                ControlResultingState = round(nanmean(ControlStateCalls((180+lengthofStimFrames+lengthofGreenFrames):((180+lengthofStimFrames+lengthofGreenFrames)+89))));
            else
            ...
            AllChOPInfo_forBinnedData(index1,1) = StartIndex; %Index for Track that = first column of each metric
            RemHere = rem(StartIndex,30);
            
            if(RemHere==0) %%%NumFrames to Add to StartPlace in subsequent columns
                AllChOPInfo_forBinnedData(index1,2) = 1;
            elseif(RemHere==1)
                AllChOPInfo_forBinnedData(index1,2) = 0;
            else
                AllChOPInfo_forBinnedData(index1,2) = (31-RemHere);
            end
            
            StartPlace_Speed = 5 + AllChOPInfo_forBinnedData(index1,2);
            StartPlace_AngSpeed = NumberofFOI + 5 + AllChOPInfo_forBinnedData(index1,2);
            binsHere = floor(NumberofFOI/30)-1;
            StopPlace_Speed = StartPlace_Speed + ((binsHere-1)*30) + 29;
            StopPlace_AngSpeed = StartPlace_AngSpeed + ((binsHere-1)*30) + 29;
            
            AllChOPInfo_forBinnedData(index1,3) = StartPlace_Speed;
            AllChOPInfo_forBinnedData(index1,4) = StopPlace_Speed;
            AllChOPInfo_forBinnedData(index1,5) = StartPlace_AngSpeed;
            AllChOPInfo_forBinnedData(index1,6) = StopPlace_AngSpeed;
