function [SpeedTests AngSpeedTests TestAvgSpeed TestControlSpeed FullControlChOPHits] = showChOPHits(folder,stimulusfile,ROImovie,stimtoIncl,C128SFlag)
    
    stimulus = load(stimulusfile);
    if(stimtoIncl==0)
        stimtoIncl= 1:1:length(stimulus(:,1));
    end
    [AllChOPHits ControlChOPHits FullControlChOPHits NumberofFOI] = summarizeChOPdata(folder,stimulusfile,ROImovie,C128SFlag,stimtoIncl);
    hitsToIncl = [];
