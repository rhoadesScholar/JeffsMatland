...
function [AllChOPHits NumberofFOI estTR estE] = summarizeChOPdata_Copper_collectHMM(folder,stimulusfile,C128SFlag,stimtoIncl)
   ...
   %%%%%%%%%%calculate HMM based on all data from all animals
    ...
    for(i=1:length(ChOPfinalTracks))
        ...
        for(j=1:numStimuli)
            ...
            Prior30sData = StateCalls(271:360);
            ...
            Prior30sData = ControlStateCalls(271:360);
            NumRoam = length(find(Prior30sData==2));
            NumDwell= length(find(Prior30sData==1));
            Total = length(Prior30sData);
            if((NumRoam/Total) > .66)
            ...
            Prior30sData = FullControlStateCalls(271:360);
            NumRoam = length(find(Prior30sData==2));
            NumDwell= length(find(Prior30sData==1));
            Total = length(Prior30sData);
            if((NumRoam/Total) > .66)
