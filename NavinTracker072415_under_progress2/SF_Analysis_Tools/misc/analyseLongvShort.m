...
function [LongvShortInfo AllLongStates AllShortStates] = analyseLongvShort(finalTracks)
...
for (i=1:length(finalTracks))
    ...
    if (firstState==1)
        ...
        for(j=1:length(longStateInd))
            if (mod(longStateInd(j),2) == 0)
                if(longStateInd(j)>1)
                longRoamingStates(index1) = longStateInd(j);
                index1 = index1+1;
                end
            end
        end
        for(j=1:length(shortStateInd))
            if (mod(shortStateInd(j),2) == 0)
                ...
                shortRoamingStates(index2) = shortStateInd(j);
                index2 = index2+1;
                end
            end
        end
 ...
 LongvShortTab(i).shortRoamingStates = shortRoamingStates; 
 LongvShortTab(i).longRoamingStates = longRoamingStates;
end
 ...
 for (i=1:length(LongvShortTab))
     ...
     for(j=1:length(LongvShortTab(i).longRoamingStates))
         stateInd = [];
         allStatesThusFar = [];
         GuessRegionStart = [];
         GuessRegionStop = [];
         stateInd = LongvShortTab(i).longRoamingStates(j);
         ...
         numFramesBeforeStart = length(find(GuessRegionStart==1));
         numFramesAfterStop = length(find(GuessRegionStop==1));
     ...
     for(j=1:length(LongvShortTab(i).shortRoamingStates))
         stateInd = [];
         allStatesThusFar = [];
         GuessRegionStart = [];
         GuessRegionStop = [];
         stateInd = LongvShortTab(i).shortRoamingStates(j);
         allStatesThusFar = stateDurationMaster(i).stateCalls(1:(stateInd-1),2);
         approxStart = floor((sum(allStatesThusFar) * 3)-10);
         approxStop = ceil(approxStart + (stateDurationMaster(i).stateCalls(stateInd,2) * 3) + 30);
         if(approxStop<(length(expStates(i).states)))
...
MasterDwLongsRevSpeedMatrix= [];
 ...
 for(i=1:length(AllShortStates(:,1)))
     ...
     roamFrames = length(find(rawStates==2));
     %%%%Get Fraction time in each states
     ratioTimeinRoamStateforShort(i) = roamFrames/totalFrames;
 ...
 for(i=1:length(AllLongStates(:,1)))
     ...
     roamFrames2 = length(find(rawStates2==2));
      %%%%Get Fraction time in each states
     ratioTimeinRoamStateforLong(i) = roamFrames2/totalFrames2;
     %%%%Get Speed, AngSpeed Data
 ...
 LongvShortInfo.avgTimeInRoamStateShort = mean(ratioTimeinRoamStateforShort);
 LongvShortInfo.avgTimeInRoamStateLong = mean(ratioTimeinRoamStateforLong);
