function plotFourHists(data1,data2,data3,data4,Name1,Name2,Name3,Name4,bins)
[N,X] = hist(data1,bins);
[N2,X2] = hist(data2,bins);
[N3,X3] = hist(data3,bins);
[N4,X4] = hist(data4,bins);

maxvalues = [X(bins) X2(bins) X3(bins) X4(bins)];
maximumV = max(maxvalues);
maxIndex = find(maxvalues==maximumV);
allfour = [1 2 3 4];
display(maxIndex)
if(length(maxIndex)>1)
    others = find(allfour~=maxIndex(1));
    othersInd = find(others~=maxIndex(2));
    others = others(othersInd);
    if(length(maxIndex)>2)
        othersInd = find(others~=maxIndex(3));
        others = others(othersInd);
    end
...
if(length(others)>0)
[Xvalues(others(1)).Ns,Xvalues(others(1)).Xs] = hist(Xvalues(others(1)).data,Xvalues(maxIndex(1)).Xs);
else
...
if(length(others)>1)
[Xvalues(others(2)).Ns,Xvalues(others(2)).Xs] = hist(Xvalues(others(2)).data,Xvalues(maxIndex(1)).Xs);
else
    [Xvalues(maxIndex(2)).Ns,Xvalues(maxIndex(2)).Xs] = hist(Xvalues(maxIndex(2)).data,Xvalues(maxIndex(1)).Xs);
    [Xvalues(maxIndex(3)).Ns,Xvalues(maxIndex(3)).Xs] = hist(Xvalues(maxIndex(3)).data,Xvalues(maxIndex(1)).Xs);
end
if(length(others)>2)
[Xvalues(others(3)).Ns,Xvalues(others(3)).Xs] = hist(Xvalues(others(3)).data,Xvalues(maxIndex(1)).Xs);
else
    [Xvalues(maxIndex(2)).Ns,Xvalues(maxIndex(2)).Xs] = hist(Xvalues(maxIndex(2)).data,Xvalues(maxIndex(1)).Xs);
end

alldata = [Xvalues(1).Ns./sum(Xvalues(1).Ns); Xvalues(2).Ns./sum(Xvalues(2).Ns); Xvalues(3).Ns./sum(Xvalues(3).Ns); Xvalues(4).Ns./sum(Xvalues(4).Ns)]';
