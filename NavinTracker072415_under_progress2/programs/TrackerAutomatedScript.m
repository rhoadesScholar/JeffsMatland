function TrackerAutomatedScript(directoryListFilename, varargin)
% TrackerAutomatedScript(directoryListFilename, [stimulusFile.txt], ['dont_track'], ['trackonly'], ['replot'], ['reanalyze'] )
...
if(nargin==0)
    disp(['Usage: TrackerAutomatedScript(argument1, [stimulusFile], other_arguments)'])
                                                                                ...
                                                                                i=i+1;
                                                                                    ...
                                                                                    i=i+1;
                                                                                        ...
                                                                                        i=i+1;
                                                                                            ...
                                                                                            sprintf('Error in TrackerAutomatedScript: Do not recognize %s',char(varargin{i}))
                                                                                            return
                                                                                        end
                                                                                    end
                                                                                end
...
if(iscell(directoryListFilename))
    for(i=1:length(directoryListFilename))
        ds = sprintf('TrackerAutomatedScript(''%s'', ''dont_track''',directoryListFilename{i});
    ...
    for(i=1:length(directoryListFilename))
        ds = 'TrackerAutomatedScript(directoryListFilename{i}';
        for(v=1:length(varargin))
            if(ischar(varargin{v}))
                ds = sprintf('%s,''%s''',ds,varargin{v});
            else
...
if(file_ptr == -1) % directoryListFilename is not an actual file, but is a directory
    ...
    if(strcmp(ext,'.avi')==1) % is a single avi file
        ...
        if(isempty(scaleRing))
            Ring = find_ring(background,sprintf('%s%s',PathName,filesep), FilePrefix, 1);
        else
        ...
        %         dummystring = 'SingleFileTrackerAutomatedScript(directoryListFilename';
        %         for(v=1:length(varargin))
            ...
            for j=1:length(movieList)
                ...
                if(does_this_file_need_making(ringfile))
                    ...
                    if(file_existence(working_file) == 0)
                        ...
                        command = sprintf('%s calc_background_ring_worm_count(''%s'', ''%s'', scaleRing);', command, PathName, moviefile);
...
for i = 1:length(directoryList(:,1))
    ...
    for j=1:length(movieList)
        ...
        if(does_this_file_need_making(ringfile))
            ...
            if(isempty(scaleRing))
                Ring = find_ring(background,sprintf('%s%s',PathName,filesep), FilePrefix, 0);
            else
                if(strcmp(Prefs.Ringtype(1:6),'square'))
                    Ring = find_ring(background,sprintf('%s%s',PathName,filesep), FilePrefix, 1);
                else
...
for k=1:size(animal_find_dud_idx,1)
    ...
    Ring = find_ring(background,sprintf('%s%s',PathName,filesep), FilePrefix);
...
for i = 1:length(directoryList(:,1))
    ...
    for j=1:length(movieList)
        ...
        if(file_existence(working_file) == 0 && ...
                ...
                does_this_file_need_making(sprintf('%s%s.BinData.mat',localpath, FilePrefix), Prefs.track_analysis_date) == 1 || ...
        ...
        if(file_existence(testfile)==0)  % this BinData file does not exist ... someone else is working on it, so let that process do the averaging
            ...
            if(trackmasterflag==1)  % but this process is a master, so wait for the jobs to finish
                ...
                if(file_existence(working_file))
                    ...
                    disp([sprintf('Neither %s nor %s exists ... consider re-running TrackerAutomatedScript for directory %s',testfile, working_file, localpath)])
                end
            end
        end
        j=j+1;
