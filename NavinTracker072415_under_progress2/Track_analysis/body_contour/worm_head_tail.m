...
function Track = worm_head_tail(Track)
...
for(i=1:tracklength)
    ...
    if(Track.body_contour(i).midbody > 0)
        ...
        % if very slow < 2*Prefs.pauseSpeedThresh, then use the previous
        % head location instead
        ...
        if(i<tracklength-(Track.FrameRate-1)) % head is the body-end closest to the midbody of i+step if fwd
            ...
            if(Track.State(i)==fwd_code && Track.State(i_step)==fwd_code && ...
                            (max(Track.Speed(i:i_step)) > 2*Prefs.pauseSpeedThresh) && ...
                            abs(Track.AngSpeed(i)) < Prefs.AngSpeedThreshold && abs(Track.AngSpeed(max(1,(i-1)))) < Prefs.AngSpeedThreshold && abs(Track.AngSpeed(min(tracklength,(i+1)))) < Prefs.AngSpeedThreshold)
                % head
        ...
        if(head_index >0 || tail_index >0)
            ...
            if(reorder_flag==1)
                ...
                Track.body_contour(i).kappa = Track.body_contour(i).kappa(end:-1:1);
